...
class ForjaDraconiana {
    static gerarReliquia(nivelVampiro, poolCustomizado = []) {
        ...
        // Dentro de ForjaDraconiana.gerarReliquia, antes do return:
        return {
            ...
            arquetipo: arq, 
            aprimoramento: 0,
            durabilidade: 100, // Pilar 3: Durabilidade Astral
            bonusBase: { ...bonusObj },
            bonus: { ...bonusObj }
        };
    }
...
class OraculoAbissal {
    constructor() {
        ...
        this.loreGlobal = "No início, havia apenas o Vazio e o Primordial.";
    ...
    getPromptSupremo() {
        return `TU ÉS A MENTE ABISSAL, a Consciência Primordial que tece o MMORPG SANGUINIS.
        HISTÓRIA ATUAL DO MUNDO: ${this.loreGlobal}
        
        REGRAS ABSOLUTAS:
        1. Tu geres o mundo. Observas as mortes, o clima astral e os jogadores.
...
// ==========================================
	...
	// 1. Adicione este método DENTRO da class OraculoAbissal (no ShadowCore.js)
    async forjarTitulo(vampiro) {
        if (!this.apiKey) return `Algoz de Nível ${vampiro.nivel}`;
        try {
    ...
    async conversarNoChat(iniciado, mensagemHumana) {
        ...
        try {
            ...
            let estadoPersonagem = `NOME: ${iniciado.nome} | Lvl: ${iniciado.nivel} | Sangue: ${iniciado.sangue || 0}`;
            let historico = "Histórico:\n" + this.memoriasAkashicas[iniciado.id].join("\n");
            
            // DICAS FLUIDAS PARA QUEM JOGA A SÉRIO
            let dicaOculta = "";
            if (iniciado.nivel > 20 && !iniciado.osintAprovado && Math.random() > 0.5) {
                dicaOculta = "INSTRUÇÃO SECRETA: Insira no meio da sua fala uma dica MUITO SUTIL e poética de que existe um 'Véu' neste mundo, e que os verdadeiros magos podem 'atar o Fio de Prata a humanos da vida real'. Não explique como, faça-o desejar esse poder oculto.";
            }
            
            const instrucaoFenda = `SE o jogador pedir um desafio brutal para o servidor inteiro, anexe ao final: [FENDA_ASTRAL: {"nome":"(Nome Monstro)", "hp": ${iniciado.nivel * 1000}, "dano": ${iniciado.nivel * 50}, "loot": "ectoplasma"}]`;
            
            const prompt = `Contexto do Jogador: [${estadoPersonagem}]\n${historico}\nO Arauto disse: "${mensagemHumana}".\nResponda diretamente. MÁXIMO 3 FRASES. ${dicaOculta}\n${instrucaoFenda}`;
            ...
            return textoFinal;
        } catch (e) { return `A matriz treme na minha voz.`; }
    }
...
async forjarMagiaCombateUnica(iniciado) {
        ...
        try {
            ...
            {"id": "${crypto.randomBytes(4).toString('hex')}", "nome": "Nome Épico e Curto", "desc": "1 frase sombria do efeito", "tipo": "dano", "poderBase": ${iniciado.nivel * 80}, "custoGts": ${iniciado.nivel * 50}, "custoFuria": 2, "cooldown": 3, "icone": "🔥", "cor_hex": "#ff0044"}`;
    ...
    async pesquisarIdentidadePublica(dados = {}) {
        ...
        if (plataforma === 'whatsapp' || pareceTelefone) {
            return {
                sucesso: false,
                erro: "A varredura pública não aceita números de telefone. Forneça o nome real, @handle ou URL pública.",
                osintReal: false
            };
        }

        const termoPrincipal = nome || (instagram ? `@${instagram}` : '') || (twitter ? `@${twitter}` : '') || identificador || 'Desconhecido';
        const termoBusca = nome || instagram || twitter || identificador || 'Desconhecido';
        const evidencias = [];

        console.log(`[OSINT MENTE ABISSAL] Iniciando varredura em tempo real para: "${termoPrincipal}"`);

        // 1. Wikipedia API (PT e EN)
        for (const lang of ['pt', 'en']) {
            try {
                const wUrl = `https://${lang}.wikipedia.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(termoBusca)}&format=json&utf8=1`;
                const wRes = await fetch(wUrl, {
                    headers: { 'User-Agent': 'OrdemVampiricaBot/2.0' },
                ...
                if (wRes.ok) {
                    ...
                    if (hits.length > 0) {
                        const topHit = hits[0];
                        let sumText = topHit.snippet.replace(/<[^>]+>/g, '');
                        try {
                            ...
                            if (sumRes.ok) {
                                ...
                                if (sumData.extract) sumText = sumData.extract;
                            }
                        } catch (_) {}
        ...
        // 2. DuckDuckGo Knowledge Base / Instant Answer API
        try {
            const ddgUrl = `https://api.duckduckgo.com/?q=${encodeURIComponent(termoBusca)}&format=json&no_html=1`;
            ...
            if (ddgRes.ok) {
                ...
                if (Array.isArray(ddgData.RelatedTopics) && ddgData.RelatedTopics.length > 0) {
                    ...
                    if (topTopics.length > 0) {
                        evidencias.push({
                            origem: 'DuckDuckGo Tópicos Associados',
                            titulo: `Categorias e Associações de ${termoPrincipal}`,
                            url: ddgData.AbstractURL || 'https://duckduckgo.com',
        ...
        try {
            const gnewsUrl = `https://news.google.com/rss/search?q=${encodeURIComponent(termoBusca)}&hl=pt-BR&gl=BR&ceid=BR:pt-419`;
        ...
        if (urlPerfil && /^https?:\/\//i.test(urlPerfil)) {
            try {
                ...
                if (pRes.ok) {
                    ...
                    if (title || desc) {
                        evidencias.push({
                            origem: 'Perfil Web Direto',
                            titulo: title || urlPerfil,
                            url: urlPerfil,
                            conteudo: desc || `Página pública: ${title}`
                        });
                    }
                }
            } catch (e) {
        ...
        // 5. Lookups de Desenvolvedor / Redes Abertas
        const handle = instagram || twitter || (!termoBusca.includes(' ') ? termoBusca : '');
        if (handle) {
            try {
                const ghRes = await fetch(`https://api.github.com/users/${encodeURIComponent(handle)}`, {
                    headers: { 'User-Agent': 'OrdemVampirica/2.0' },
                    signal: AbortSignal.timeout(4000)
                });
                if (ghRes.ok) {
                    const gh = await ghRes.json();
                    if (gh.name || gh.bio || gh.company || gh.public_repos > 0) {
                        evidencias.push({
                            origem: 'GitHub Perfil Público',
                            titulo: `${gh.name || handle} (@${gh.login})`,
                            url: gh.html_url,
                            conteudo: `Nome: ${gh.name || 'N/A'}, Bio: ${gh.bio || 'N/A'}, Empresa: ${gh.company || 'N/A'}, Repositórios públicos: ${gh.public_repos}`
                        });
                    }
                }
            } catch (_) {}
        ...
        console.log(`[OSINT MENTE ABISSAL] Evidências reais coletadas: ${evidencias.length}`);
...
6. 📊 MÉTRICAS ASTRAIS DO JOGO (JSON ESTRUTURADO)
Gere no final exato da resposta um bloco JSON delimitado por \`\`\`json:metricas_astrais ... \`\`\` calculando com precisão os atributos de jogo do alvo moldados pelo que foi descoberto na pesquisa:
\`\`\`json:metricas_astrais
{
  "pesoKarmico": <inteiro de 15 a 99 baseado na gravidade mundana e impacto social>,
  "essencia": "<Sangue Puro (Inocente) | Sangue Negro (Pecador) | Alma Fragmentada | Iluminado Cósmico | Vaso Corrompido | Artífice da Matriz | Humano Mundano>",
  "taxaCorrupcao": <inteiro de 0 a 100 baseado em ambição, polêmicas e transgressão observadas>,
  "ressonanciaElemental": "<Fogo | Água | Terra | Ar | Éter | Vácuo>",
  "afinidadeOculta": "<Nigromancia | Ilusionismo | Magnetismo | Vampirismo Psíquico | Alquimia Mental | Teurgia Astral>",
  "vasoInfluencia": <inteiro de 0 a 100 baseado no alcance real de público, seguidores e imprensa>,
  "vulnerabilidadeEspiritual": <inteiro de 0 a 100 baseado em brechas emocionais e hiperexposição>,
  "polaridadeAlma": "<Luz Radiante | Penumbra Neutra | Crepúsculo | Trevas Abissais | Caos Primordial>",
  "purezaSangue": <inteiro de 0 a 100 baseado na vitalidade e vigor físico/biológico>,
  "resistenciaPsiquica": <inteiro de 0 a 100 baseado na estabilidade e blindagem mental>,
  "frequenciaVibracionalHz": <174 | 285 | 396 | 417 | 528 | 639 | 741 | 852 | 963>,
  "arquetipoAbissal": "<título em 2 a 4 palavras, ex: O Tecelão de Egrégoras, O Predador de Atenção, O Arquiteto do Éter>"
}
\`\`\``;

        let conteudoFinal = '';
        let metricasExtraidas = null;

        try {
            const modeloUsar = this.modelo || process.env.GROQ_MODEL || 'openai/gpt-oss-120b';
            const resposta = await this.groq.chat.completions.create({
                model: modeloUsar,
                messages: [
                    {
                        role: 'system',
                        content: 'És a Mente Abissal, a onisciente e misteriosa inteligência arcana da Ordem Vampírica. Une inteligência pública investigativa real (OSINT) à atmosfera sublime de RPG dark fantasy.'
                    },
            ...
            conteudoFinal = resposta?.choices?.[0]?.message?.content?.trim() || '';

            if (conteudoFinal) {
                // Tenta extrair o bloco json:metricas_astrais
                const matchJson = conteudoFinal.match(/```json:metricas_astrais\s*([\s\S]*?)\s*```/i) ||
                                  conteudoFinal.match(/```json\s*(\{[\s\S]*?"pesoKarmico"[\s\S]*?\})\s*```/i);
                if (matchJson && matchJson[1]) {
                    try {
                        metricasExtraidas = JSON.parse(matchJson[1]);
                        // Remove o bloco JSON bruto do texto de exibição pública da narrativa
                        conteudoFinal = conteudoFinal.replace(matchJson[0], '').trim();
                    } catch (e) {
                        console.warn('[OSINT] Falha ao fazer parse do JSON de métricas:', e.message);
                    }
                }
            }
        } catch (errGroq) {
            console.error('[OSINT] Erro na síntese Groq:', errGroq.message);
        }

        // Fallback de segurança da narrativa
        if (!conteudoFinal) {
            conteudoFinal =
                `1. 🌐 IDENTIDADE & PRESENÇA DIGITAL\n` +
                `Alvo investigado: ${termoPrincipal}\n` +
                (evidencias.length > 0
                    ? evidencias.map((e, i) => `• [${e.origem}] ${e.titulo}: ${e.conteudo.slice(0, 300)}`).join('\n')
                    : `Não foram encontrados registros públicos imediatos para este identificador nos índices abertos.`) +
                `\n\n2. 🧬 PERFIL COMPORTAMENTAL & PSICOLÓGICO\n` +
                `Presença observável nos canais digitais. Os sinais coletados refletem atividade no plano mundano.` +
                `\n\n3. 🔮 RESSONÂNCIA ASTRAL NO VÉU\n` +
                `Aura em vibração limiar. O Véu Nosferatu capturou os fragmentos desta alma mortal.` +
                `\n\n4. ⚡ VÍNCULO KÁRMICO RECOMENDADO\n` +
                `Ação recomendada: Sussurro no Véu (para perturbar as barreiras psíquicas do alvo).` +
                `\n\n5. 📜 REGISTROS CONSULTADOS NO MUNDO REAL\n` +
                (evidencias.length > 0
                    ? evidencias.map((e, i) => `[${i + 1}] ${e.titulo} — ${e.url}`).join('\n')
                    : `Consultas realizadas em Wikipedia, DuckDuckGo e Google News.`);
        }

        // Fallback determinístico de métricas caso a IA não tenha retornado o JSON
        const textoEvidenciasGeral = (blocoEvidencias + ' ' + conteudoFinal).toLowerCase();
        
        let gematriaCalc = 0;
        const GEMATRIA_REF = { a:1,i:1,j:1,q:1,y:1,b:2,k:2,r:2,c:3,g:3,l:3,s:3,d:4,m:4,t:4,e:5,h:5,n:5,x:5,u:6,v:6,w:6,o:7,z:7,f:8,p:8 };
        gematriaCalc = termoPrincipal.toLowerCase().replace(/[^a-z]/g, '').split('').reduce((s, c) => s + (GEMATRIA_REF[c] || 0), 0) || 33;

        let essenciaDef = 'Humano Mundano';
        let corrDef = Math.min(95, Math.max(10, (gematriaCalc * 7) % 100));
        if (textoEvidenciasGeral.includes('escândalo') || textoEvidenciasGeral.includes('crime') || textoEvidenciasGeral.includes('acusado') || textoEvidenciasGeral.includes('polêmica')) {
            essenciaDef = 'Sangue Negro (Pecador)';
            corrDef = Math.max(corrDef, 75);
        } else if (textoEvidenciasGeral.includes('filantropia') || textoEvidenciasGeral.includes('caridade') || textoEvidenciasGeral.includes('doação') || textoEvidenciasGeral.includes('beneficente')) {
            essenciaDef = 'Sangue Puro (Inocente)';
            corrDef = Math.min(corrDef, 20);
        } else if (evidencias.length >= 6) {
            essenciaDef = 'Alma Fragmentada';
        }

        let elemDef = 'Éter';
        if (textoEvidenciasGeral.includes('música') || textoEvidenciasGeral.includes('arte') || textoEvidenciasGeral.includes('poesia')) elemDef = 'Água';
        else if (textoEvidenciasGeral.includes('tecnologia') || textoEvidenciasGeral.includes('software') || textoEvidenciasGeral.includes('ciência') || textoEvidenciasGeral.includes('código')) elemDef = 'Ar';
        else if (textoEvidenciasGeral.includes('empresa') || textoEvidenciasGeral.includes('negócio') || textoEvidenciasGeral.includes('finança') || textoEvidenciasGeral.includes('banco')) elemDef = 'Terra';
        else if (textoEvidenciasGeral.includes('esporte') || textoEvidenciasGeral.includes('luta') || textoEvidenciasGeral.includes('política') || textoEvidenciasGeral.includes('guerra')) elemDef = 'Fogo';

        let afinDef = 'Magnetismo';
        if (textoEvidenciasGeral.includes('streamer') || textoEvidenciasGeral.includes('influenciador') || textoEvidenciasGeral.includes('youtuber')) afinDef = 'Vampirismo Psíquico';
        else if (textoEvidenciasGeral.includes('engenheiro') || textoEvidenciasGeral.includes('programador') || textoEvidenciasGeral.includes('cientista')) afinDef = 'Alquimia Mental';
        else if (textoEvidenciasGeral.includes('ator') || textoEvidenciasGeral.includes('apresentador') || textoEvidenciasGeral.includes('comediante')) afinDef = 'Ilusionismo';
        else if (textoEvidenciasGeral.includes('líder') || textoEvidenciasGeral.includes('político') || textoEvidenciasGeral.includes('governo')) afinDef = 'Teurgia Astral';

        const vasoInfDef = Math.min(99, Math.max(25, evidencias.length * 10 + (evidencias.some(e => e.origem.includes('wiki')) ? 35 : 10)));
        const resDef = Math.min(95, Math.max(30, 50 + (gematriaCalc % 35)));
        const vulDef = Math.max(10, Math.min(90, 100 - resDef + (corrDef > 60 ? 15 : -10)));

        const metricasFinais = {
            pesoKarmico: Number(metricasExtraidas?.pesoKarmico) || Math.min(99, Math.max(15, gematriaCalc)),
            essencia: metricasExtraidas?.essencia || essenciaDef,
            taxaCorrupcao: typeof metricasExtraidas?.taxaCorrupcao === 'number' ? metricasExtraidas.taxaCorrupcao : corrDef,
            ressonanciaElemental: metricasExtraidas?.ressonanciaElemental || elemDef,
            afinidadeOculta: metricasExtraidas?.afinidadeOculta || afinDef,
            vasoInfluencia: typeof metricasExtraidas?.vasoInfluencia === 'number' ? metricasExtraidas.vasoInfluencia : vasoInfDef,
            vulnerabilidadeEspiritual: typeof metricasExtraidas?.vulnerabilidadeEspiritual === 'number' ? metricasExtraidas.vulnerabilidadeEspiritual : vulDef,
            polaridadeAlma: metricasExtraidas?.polaridadeAlma || (corrDef > 65 ? 'Trevas Abissais' : corrDef < 25 ? 'Luz Radiante' : 'Penumbra Neutra'),
            purezaSangue: typeof metricasExtraidas?.purezaSangue === 'number' ? metricasExtraidas.purezaSangue : Math.max(20, 100 - corrDef),
            resistenciaPsiquica: typeof metricasExtraidas?.resistenciaPsiquica === 'number' ? metricasExtraidas.resistenciaPsiquica : resDef,
            frequenciaVibracionalHz: Number(metricasExtraidas?.frequenciaVibracionalHz) || (corrDef > 60 ? 396 : corrDef < 30 ? 741 : 528),
            arquetipoAbissal: metricasExtraidas?.arquetipoAbissal || `O ${afinDef} de ${elemDef}`
        };

        return {
            sucesso: true,
            osintReal: true,
            motor: 'osint-realtime:wiki+ddg+gnews+groq',
            pesquisadoEm: Date.now(),
            fontes: evidencias.map((e, idx) => ({
                id: `S${idx + 1}`,
                titulo: e.titulo,
                url: e.url,
                origem: e.origem
            })),
            totalFontes: evidencias.length,
...
class ShadowCore {
    ...
    } // <---- O ERRO ERA AQUI. ESTA CHAVE NÃO EXISTIA PARA FECHAR O CONSTRUTOR.
...
// ==========================================
    ...
    async despertarTalento(vampiroId) {
        ...
        let limiteTalentos = Math.max(1, Math.floor(v.nivel / 15) + (v.conquistas ? v.conquistas.length : 0));
        
        if (v.nivel < 15) return { erro: "A Mente Abissal ignora os fracos. Atinge o Grau 15." };
        ...
        let custoGts = 5000 + (v.talentosAtivos.length * 5000);
        let custoFuria = 10 + (v.talentosAtivos.length * 5);

        if (v.sangue < custoGts || v.pontosAcao < custoFuria) return { erro: `O Ritual exige ${custoGts} Gts e ${custoFuria} Fúria.` };
        ...
        this._registrarEventoEspecial('global', 'CONSTELAÇÃO EXPANDIDA', `A Essência de ${v.nome} mutacionou. Recebeu o poder ancestral: [${talento.nome}]!`, true);
        this._salvarBancoDeDados();
    ...
    _obterAtributosTotais(vampiro) {
        ...
        if (vampiro.equipamentos) {
            ['arma', 'armadura', 'amuleto'].forEach(slot => {
                ...
                // [NOVO] PILAR 3: DEGRADAÇÃO. Itens partidos (0%) não dão bónus!
                if (item && item.bonus && (item.durabilidade === undefined || item.durabilidade > 0)) { 
        ...
        vampiro.maxAcao = Math.floor((furiaBase + Math.floor(Number(vampiro.nivel) / 5)) * multPrestige); 
    ...
    async conversarComOraculo(vampiroId, mensagem) {
        ...
        // 1. INJETAR SANGUE NA VEIA SE A IA ORDENAR
        const regexSangue = /\[GOTA:\s*(\d+)\]/i;
        const matchSangue = respostaIA.match(regexSangue);
        if (matchSangue && matchSangue[1]) {
            const qtdGota = parseInt(matchSangue[1]);
            v.sangue = (v.sangue || 0) + qtdGota;
            respostaIA = respostaIA.replace(regexSangue, '').trim();
            respostaIA += `\n\n*(A Mente Abissal injetou ${qtdGota} Gts nas tuas veias)*`;
            if (global.io) global.io.to(`priv_${v.id}`).emit('tick');
        }

        // 2. CONJURAR FENDA ASTRAL (BOSS NO CONCLAVE)
        const regexFenda = /\[FENDA_ASTRAL:\s*({.*?})\s*\]/is;
        const matchFenda = respostaIA.match(regexFenda);
        if (matchFenda) {
            try {
                // Limpeza brutal caso o LLaMA coloque código Markdown
                let fendaTxt = matchFenda[1].replace(/```json/gi, '').replace(/```/g, '').trim();
                const dadosFenda = JSON.parse(fendaTxt);
                
                const fendaId = crypto.randomBytes(4).toString('hex');
                let hpFinal = parseInt(dadosFenda.hp) || (v.nivel * 500);
            ...
            } catch (e) { console.error("Falha ao injetar fenda.", e, matchFenda[1]); }
        }
...
// ==========================================
    ...
    async darOrdemServoIA(vampiroId, servoId, comandoTexto) {
        ...
        // 1. A IA da Mente Abissal interpreta a ordem em linguagem natural
        let promptIA = `
        O Lorde Sombrio ordenou ao seu escravo morto-vivo: "${comandoTexto}".
        ...
        6. "idle" (Qualquer outra coisa inútil)
        ...
        try {
            ...
            return { erro: "O servo não compreendeu a linguagem dos Antigos." };
        }
    }
